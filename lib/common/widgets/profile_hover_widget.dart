...
class _ProfileHoverWidgetState extends State<ProfileHoverWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    List<MenuModel> list = [
      MenuModel(icon: Images.profile, title: getTranslated('profile', context), route: ()=> RouteHelper.getProfileScreen()),
      MenuModel(icon: Images.order, title: getTranslated('my_orders', context), route: ()=> RouteHelper.getOrderListScreen()),
      MenuModel(
        icon: Images.profile, title: getTranslated('log_out', context),
        route: (){
    ...
    return MouseRegion(
      ...
      child: Container(color: Theme.of(context).cardColor, child: Column(
        ...
        children: list.map((item) => widget.currentRoute == _checkRoutes(context, item.title) ? const SizedBox() : InkWell(
          onTap: (){
            setState(() {
              isExited = true;
            });

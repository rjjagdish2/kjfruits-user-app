...
class MyInAppBrowser extends InAppBrowser {
  ...
  void onExit() {
    if(_canRedirect) {
     RouteHelper.getOrderSuccessRoute(orderId.toString(), 'payment-fail', action: RouteAction.pushReplacement);
    }
  ...
  void _pageRedirect(String url) {
    if(_canRedirect) {
      ...
      if(isSuccess){
        ...
        if(isWallet){
          RouteHelper.getWalletRoute(token: token, status: 'success', action: RouteAction.pushReplacement);
        }else if(isOrderDetails){
          RouteHelper.getOrderDetailsRoute(orderId.toString(), action: RouteAction.pushReplacement);
        }else{
          if(token.isNotEmpty) {
        ...
        RouteHelper.getWalletRoute(token: 'failed',status: 'failed', action: RouteAction.pushNamedAndRemoveUntil);

      }else if(isFailed) {
        RouteHelper.getOrderSuccessRoute('No', 'payment-fail', action: RouteAction.pushReplacement);
      }else if(isCancel) {
        RouteHelper.getOrderSuccessRoute(orderId.toString(), 'payment-cancel', action: RouteAction.pushReplacement);
      }
    }

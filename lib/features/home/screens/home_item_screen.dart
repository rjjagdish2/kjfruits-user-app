...
class _HomeItemScreenState extends State<HomeItemScreen> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_){
      Provider.of<ProductProvider>(context, listen: false).getItemList(1, isUpdate: false, productType: widget.productType);

      if((Provider.of<SplashProvider>(context, listen: false).configModel?.flashDealProductStatus ?? false) && (Provider.of<FlashDealProvider>(context, listen: false).flashDealModel == null)) {
        Provider.of<FlashDealProvider>(context, listen: false).getFlashDealProducts(1, isUpdate: false);
      }
    });
    super.initState();
  }

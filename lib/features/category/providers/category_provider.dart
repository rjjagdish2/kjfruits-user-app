...
import 'package:flutter_grocery/features/category/domain/models/category_product_model.dart';
...
class CategoryProvider extends ChangeNotifier  {
  ...
  void getCategoryProductList(String categoryID, int offset) async {
    ApiResponseModel apiResponse = await categoryRepo.getCategoryProductList(categoryID, offset);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _categoryProductList = CategoryProductModel.fromJson(apiResponse.response!.data);
  ...
  void initCategoryProductList(String id, int offset) async {
    _hasData = true;
    ApiResponseModel apiResponse = await productRepo.getBrandOrCategoryProductList(id, offset);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      
      if(offset == 1){
        _subCategoryProductList = CategoryProductModel.fromJson(apiResponse.response!.data);
      }else{
        _subCategoryProductList?.products?.addAll(CategoryProductModel.fromJson(apiResponse.response!.data).products!);
        _subCategoryProductList?.offset = CategoryProductModel.fromJson(apiResponse.response!.data).offset;
        _subCategoryProductList?.totalSize = CategoryProductModel.fromJson(apiResponse.response!.data).totalSize;
      }
  ...
  void sortCategoryProduct(int filterIndex) {
    if(filterIndex == 0) {
      _subCategoryProductList?.products?.sort((product1, product2) => product1.price!.compareTo(product2.price!));
    }else if(filterIndex == 1) {
      _subCategoryProductList?.products?.sort((product1, product2) => product1.price!.compareTo(product2.price!));
      Iterable iterable = (_subCategoryProductList?.products ?? []).reversed;
      _subCategoryProductList?.products = iterable.toList() as List<Product>;
    }else if(filterIndex == 2) {
      _subCategoryProductList?.products?.sort((product1, product2) => product1.name!.toLowerCase().compareTo(product2.name!.toLowerCase()));

...
class CategoryRepo extends DataSyncRepo{
  ...
  Future<ApiResponseModel> getCategoryProductList(String categoryID, int offset) async {
    try {
      final response = await dioClient.get('${AppConstants.categoryProductUri}$categoryID?limit=10&&offset=$offset');
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }

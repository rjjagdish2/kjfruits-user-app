...
class OnBoardingScreen extends StatelessWidget {
            ...
            Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              TextButton(
                onPressed: () {
                  ...
                  context.go('${RouteHelper.menu}?from_intro=${jsonEncode(true)}');
                },
                child: Text(
  ...
  List<Widget> _pageIndicators(var onBoardingList, BuildContext context) {
    ...
    for (int i = 0; i < onBoardingList.length; i++) {
      indicators.add(
        Container(
          width: i == Provider.of<OnBoardingProvider>(context, listen: false).selectedIndex ? 20 : 10,
          height: 10,
          margin: const EdgeInsets.only(right: 5),
          decoration: BoxDecoration(
            color:
                i == Provider.of<OnBoardingProvider>(context, listen: false).selectedIndex ? Theme.of(context).primaryColor : ColorResources.getGreyColor(context),

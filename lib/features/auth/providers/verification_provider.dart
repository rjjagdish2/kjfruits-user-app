...
class VerificationProvider with ChangeNotifier {
  ...
  Future<ResponseModel> checkPhone(BuildContext context, String phone, String fromPage) async {
    _isLoading = true;
    notifyListeners();
     ...
     // String? routeName = ModalRoute.of(Get.context!)?.settings.name;
      ...
      if(isReplaceRoute){
        if(fromPage == FromPage.profile.name){
         ...
         RouteHelper.getVerifyRoute(phone, fromPage, action: RouteAction.pushReplacement);
      }else{
        if(fromPage == FromPage.profile.name){
          Navigator.pop(Get.context!);
        }
        RouteHelper.getVerifyRoute(phone, fromPage);
      }
  ...
  Future<ResponseModel> checkEmail(BuildContext context, String email, String fromPage) async {
    _isLoading = true;
    resendLoadingStatus = true;
    notifyListeners();
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if(isReplaceRoute){
        ...
        RouteHelper.getVerifyRoute(email, fromPage, action: RouteAction.pushReplacement);
      }else{
        if(fromPage == FromPage.profile.name){
          Navigator.pop(Get.context!);
        }
        RouteHelper.getVerifyRoute(email, fromPage);
  ...
  Future<void> firebaseVerifyPhoneNumber(BuildContext context, String phoneNumber, String fromPage, {bool isForgetPassword = false})async {
    _isLoading = true;
    notifyListeners();
    ...
    FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationFailed: (FirebaseAuthException e) {
        if(Navigator.canPop(context)) {
          if(!(ModalRoute.of(context)?.settings.name == RouteHelper.profileEdit) && !(ModalRoute.of(context)?.settings.name == RouteHelper.sendOtp)){
            Navigator.pop(Get.context!);
          }
        }
        _isLoading = false;
      ...
      codeSent: (String vId, int? resendToken) {
        ...
        bool isReplaceRoute = ModalRoute.of(context)?.settings.name == RouteHelper.verification;
        ...
        if(fromPage == FromPage.profile.name){
          if(isReplaceRoute){
            RouteHelper.getVerifyRoute(phoneNumber, fromPage, session: vId, action: RouteAction.pushReplacement);
          }else{
            RouteHelper.getVerifyRoute(phoneNumber, fromPage, session: vId,);
          }
        }else{
          if(isReplaceRoute){
            RouteHelper.getVerifyRoute(phoneNumber,
                isForgetPassword ? FromPage.forget.name : fromPage, session: vId, action: RouteAction.pushReplacement);
          }else{
            RouteHelper.getVerifyRoute(phoneNumber,

...
class OrderDetailsButtonView extends StatelessWidget {
  ...
  const OrderDetailsButtonView({super.key, required this.orderModel, required this.phoneNumber});
        ...
        return Column(children: [
                ...
                orderProvider.trackModel!.orderStatus == 'pending' ? Expanded(child: Padding(
                  ...
                  child: TextButton(
                    style: TextButton.styleFrom(
                      ...
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(width: 2, color: Theme.of(context).disabledColor.withValues(alpha: 0.1))),
                    ),
                    onPressed: () {
          ...
          (orderProvider.trackModel!.orderStatus == 'confirmed' || orderProvider.trackModel!.orderStatus == 'processing' || orderProvider.trackModel!.orderStatus == 'out_for_delivery') ? Padding(
            ...
            child: CustomButtonWidget(
              buttonText: 'order_track'.tr,
              onPressed: () {
                RouteHelper.getOrderTrackingRoute(orderProvider.trackModel!.id, phoneNumber);
              },
            ),
          ) : const SizedBox(),
              ...
              child: Container(
                ...
                child: CustomButtonWidget(buttonText: 'chat_with_delivery_man'.tr, onPressed: (){
                  RouteHelper.getChatRoute(
                    ...
                    profileImage:  orderProvider.trackModel?.deliveryMan?.image ?? "",
                    userName: "${orderProvider.trackModel?.deliveryMan?.fName ?? ""} ${orderProvider.trackModel?.deliveryMan?.lName ?? ""}",

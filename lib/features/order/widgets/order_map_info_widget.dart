...
class OrderMapInfoWidget extends StatefulWidget {
  ...
  const OrderMapInfoWidget({super.key, required this.address});
...
class _OrderMapInfoWidgetState extends State<OrderMapInfoWidget> {
  ...
  void initState() {
    ...
    _latLng = LatLng(widget.address!.latitude!, widget.address!.longitude!);
  ...
  void _setMarker() async {
     await BitmapDescriptor.asset(const ImageConfiguration(size: Size(30, 50)), Images.restaurantMarker).then((marker) {
      _markers = {};
      _markers.add(Marker(
